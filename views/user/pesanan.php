                    ...
                    <?php if (!empty($pesanan)): ?>
                        <?php foreach ($pesanan as $p): ?>
                            <tr class="border-b hover:bg-gray-100">
                                <td class="px-6 py-4"><?= htmlspecialchars($p['nama_peminta']) ?></td>
                                <td class="px-6 py-4"><?= htmlspecialchars($p['nama_item']) ?></td>
                                <td class="px-6 py-4"><?= htmlspecialchars($p['jumlah']) ?></td>
                                <td class="px-6 py-4"><?= htmlspecialchars($p['satuan']) ?></td>
                                <td class="px-6 py-4"><?= htmlspecialchars($p['keterangan']) ?></td>
                                <td class="px-6 py-4 <?= $p['status_barang'] === 'waiting confirmation' ? 'text-red-500' : ($p['status_barang'] === 'confirmed' ? 'text-blue-500' : ($pesanan['status_barang'] === 'on process' ? 'text-purple-500' : 'text-green-500')); ?>">
                                    <?= htmlspecialchars($p['status_barang']); ?>
                                </td>
                                <td class="px-6 py-4 <?= $p['status'] === 'Not Approve' ? 'text-red-500' : ($p['status'] === 'Approve' ? 'text-green-500' : ''); ?>">
