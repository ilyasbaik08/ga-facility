                    ...
                    <?php if (!empty($pesanan)): ?>
                        <?php foreach ($pesanan as $pesanan): ?>
                            <tr class="border-b hover:bg-gray-100">
                                ...
                                <td class="px-6 py-4"><?= htmlspecialchars($pesanan['nama_item']) ?></td>
                                ...
                                <td class="px-6 py-4 <?= $pesanan['status_barang'] === 'waiting confirmation' ? 'text-red-500' : ($pesanan['status_barang'] === 'confirmed' ? 'text-blue-500' : ($pesanan['status_barang'] === 'on process' ? 'text-purple-500' : 'text-green-500')); ?>">
                                    <?= htmlspecialchars($pesanan['status_barang']); ?>
                                </td>
                                <td class="px-6 py-4">
                                    <?php if ($pesanan['status_barang'] === "request"): ?>
                                        <form action="../../controller/ConfirmPesananController.php" method="POST">
                                            <input type="hidden" name="user_id" value="<?= $pesanan['id']; ?>">
                                ...
                                <td class="px-6 py-4">
                                <?php elseif ($pesanan['status'] === "Not Approve"): ?>
                                    <form action="../../controller/ConfirmPesananController.php" method="POST">
                                        <input type="hidden" name="user_id" value="<?= $pesanan['id'] ?>">
                                ...
                                <?php elseif ($pesanan['status'] === "Approve"): ?>
                                    <span class="text-green-500">Completed</span>
                                <?php endif; ?>
                                </td>
                            </tr>
